<template>
  <div id="share">
    <div class="share-box" :class="{ mobile: mobileLayout }">
      ...
      <a title="QQ空间" rel="nofollow" class="share-link qzone" @click.prevent="shareWindow(`http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=${url}&title=${title()}&summary=${description()}&site=${url}`)">
...
export default {
  ...
  computed: {
    url () {
      return `https://zzcit.com${this.$route.fullPath}`
    },
    mobileLayout () {
  ...
  methods: {
    ...
    title () {
      ...
      catch (err) { return 'zzcit.com' }
    },
    shareWindow (url) {
      url = url.includes('mailto') ? url.replace(/\s|\||zzcit.com/g, '') : encodeURI(url)
      // console.log(url)
      // 给打开的窗口命名
      let winName = "newWin"
